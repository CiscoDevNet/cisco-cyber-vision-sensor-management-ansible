...
#### Switches- IE3300, IE3400, IE3500, IE9320, and Catalyst 9300
...
- **IE9300**: Includes the IE9320 and IE3500*
...
*Note: the IE3500 IOS version is more similar to the Catalyst9300 or IE9300 series, so requires the IE9300 playbook for Ansible CV Sensor installation.
    ...
    <DEVICE_TYPE>:            # The supported device type this host belongs to, such as IE3X00, IR1XXX, etc.
      hosts:
        <IP_ADDRESS>:         # The IP address of the specific host device  
          ...
          active_discovery:   # To enable Active discovery feature when active discovery tar file is used
...
These files hold variables that are used by the playbooks.  The sample `vars.yml` files included here require this information in order for the playbooks to function:
...
This file holds authentication credentials, keys, tokens, and other sensitive information that should remain encrypted.  To edit this file it must first be decrypted:
...
Access to the placeholder `secrets.yml` file is guarded by a default password, 'Cisco12345!', ***it is crucial that this password be reset by the user to ensure the security of your Ansible Vault contents.***  Reseting the password can be accomplished with the following command:

```bash
ansible-vault rekey '/path/to/secrets.yml' '/path/to/secrets2.yml'
```

You will then be prompted to authenticate using the current password and can then enter your new password.

(Note that each `secrets.yml` file is encrypted with its own password and they must each be reset individually, or as a group as seen above.)

Conversely, you can create a fresh secrets file to encrypt with your own password.  This can be done by first creating an unencrypted file and then encrypting it using:
...
You will then need to manually enter the fields shown above and save the new secrets file.
